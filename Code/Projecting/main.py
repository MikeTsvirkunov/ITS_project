...
kcm_extraction_model = tf.keras.models.load_model('../../PipeLines/Coder/coder_from_spacy_to_kcm_onh5.h5')
...
def sendStudentJson(description=Body()):
    ...
    wp_of_event, wp_of_event_vectorized = get_vectorized_wp_and_wp(sentences=description['event_description'].split('.'), 
                                                                   wp_getter=get_wp_in_line_hard,
    ...
    predict = kcm_extraction_model.predict(descriptors_vectorized[distances[:1]])
    skills = dict()
